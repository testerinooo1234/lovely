...
- **Natural collocations / verb-first kink framing:** do not reify yes/no/stop/ask into protocol nouns with the wrong verb (`cheated on the stop`, `proud of the no`). Keep verbs as verbs. For cuckold telling beats, prefer active verbs aimed at the sub (`making you hear it`) over a reified SKU (`the report`) — "making" carries the dominance/mean half. Vary the surface phrasing; don't invent a new house noun. See `.cursor/rules/story-writing.mdc` §9.
- **No obscure house jargon / worn-out stock phrases:** do not mint private codes (`soft things`, `soft steps`, `soft reports`, `soft restart`, `the report` as a reusable kink label) that mean “take it slow” or “tell him after” without saying so. Name the act (coffee only, kissing only, making him hear every detail). Do not copy-paste the same witty excuse or metaphor across every escalation beat — vary with clearer meaning or cut. Intentional cutting nicknames (`soft boy`) are fine. See `.cursor/rules/story-writing.mdc` §10.
