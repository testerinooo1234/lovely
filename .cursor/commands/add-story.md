...
- **Cuckold “gaslighting” (craft term only):** when the brief asks for gaslighting / soft reality-rewrite / moved goalposts, use the loving rename-and-assent technique in `.cursor/rules/story-writing.mdc` §13. **Never** put `gaslight` / `gaslighting` in narration, dialogue, tags, titles, or excerpts — show the rewrite (cheating → care, jealousy → the problem, his “yes” after the fact).
